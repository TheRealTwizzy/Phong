...
export const ScoreBoard: React.FC<ScoreBoardProps> = ({
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1.5 leading-none mt-0.5">
              ...
              <span
                id="scoreboard-daily-streak-pill"
                className="inline-flex items-center gap-0.5 text-[8px] sm:text-[9px] font-mono font-bold px-1 rounded bg-amber-500/20 text-amber-300 border border-amber-500/30"
                title={`${profile?.dailyStreak || 1} day consecutive login streak!`}
              >
                <Flame className="w-2.5 h-2.5 text-amber-400 fill-amber-400 animate-pulse" />
                <span>{profile?.dailyStreak || 1}d</span>
              </span>
            </div>
          </div>
        </button>

        {/* Missions Button with Notification Badge */}
        <button
          id="btn-open-daily-missions"
          onClick={onOpenMissions}
          title={t('missions_title', language)}
          className="relative p-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-700 text-cyan-400 border border-slate-700 transition active:scale-95 flex items-center justify-center"
        >
          <Target className="w-4 h-4" />
          {unclaimedMissionsCount > 0 && (
            <span
              id="missions-badge-counter"
              className="absolute -top-1 -right-1 min-w-4 h-4 px-1 rounded-full bg-rose-500 text-white text-[9px] font-black font-mono flex items-center justify-center border border-slate-900 animate-bounce"
            >
      ...
      {/* Center: Score Display & Rally Counter */}
      ...
      <div className="flex items-center gap-1 sm:gap-1.5">
        <button
          ...
          title={t('match_history_title', language)}
        ...
        <button
          ...
          title={t('leaderboard_title', language)}
          className="p-1.5 rounded-lg bg-zinc-800/80 hover:bg-zinc-700 text-amber-400 border border-zinc-700 transition active:scale-95"
        >
        ...
        <button
          ...
          className="p-1.5 rounded-lg bg-zinc-800/80 hover:bg-zinc-700 text-purple-400 border border-zinc-700 transition active:scale-95"
        >
