...
- **Serving is aimed**: a drag from the paddle sets direction and power (`serveVelocity`, bounded by `serveAngleMax`/`servePowerMax` and the match speed band); a plain tap still serves. `autoServeSeconds` fires a held serve by itself so a PvP match can't stall.
