...
- **A session cookie** (`phong_session`, HMAC-signed, 12h) minted per load by `POST /api/session`. The profile records the one `activeSessionId` that owns it; the newest load wins, since the player is sitting at the device they just opened. Every write (`match/record`, `practice/record`, `missions/claim|reroll`, `profile/initialize|me|claim`, avatar) sits behind `requireActiveSession`, and profile **reads** behind `blockReleasedDevice` so a retired device can never reach the lazy mint. The WS upgrade enforces the same rule — barred from recording a solo match, an evicted device could otherwise still play a duel, which the relay records itself.
- **Release tombstones** (`released_devices`): a claim records the device it took the account *from*, so the server can tell "a player whose account is alive elsewhere" from "a browser I have never met" and say so. `POST /api/session/reset` is the only way off a released device — a **new** device identity and an empty profile, never the account back.
...
**Every deployment force-refreshes session auth, by construction** (`server/build.ts`). A session token carries the `buildId` it was minted under and is signed over it, so a session from a build that is not the one being served resolves to `stale_build`; the client reloads onto the current deployment (guarded by `sessionStorage` so it cannot loop) and re-mints. The device cookie is untouched by all of this — **sessions are disposable, the device→account binding is not** — so a forced refresh never costs anybody their account. The id is `BUILD_ID` from the environment, else a digest of the built `dist/index.html` (Vite rewrites the hashed asset names into it every build, so it changes exactly when the client changes and **not** when the same build merely restarts — a crash-loop must not log the field out), else a per-process value in dev. `index.html` is served `Cache-Control: no-cache`, or a client told it was stale would reload straight back onto the build it was already running. **This is the standing rule for the project, and it holds for every future update without anyone remembering to bump anything.**
