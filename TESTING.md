...
| Suite | Owns |
|---|---|
| `rating` `xp` `achievements` | TrueSkill, tiers — including the ranked-duel count the apex asks for beyond the rating — `tierProgress` (incl. the full top band), the per-rung solo caps, the arrow-count bands, the XP curve, solo momentum/fatigue, the achievement tree and the unlocks it gates |
...
| `db` `apexGate` | The store: matches, idempotency, abandons, and the counters `recordMatch` derives — `apexGate` for the ranked-duel count (which results add to it, the held-back Legend, the promotion on the 25th), the back-filled `duel_25`, `wall_500`, and a win-streak task advancing off the pooled counter |
...
| `missions` `titles` | The dealt hand, paid rerolls and the capped free re-deal, all eight task kinds, elite unlocks into both catalogues (cosmetics and titles), Practice Wall XP and the day's return COUNT it banks |
...
| `botMatch` | The play-bot simulation with no socket, no room and no clock: that a match between two bots TERMINATES (a simulation that stalls is a bot that never frees its seat), that rallies happen rather than an unbroken run of aces, that the stronger true skill wins more often, and that the current run and its peak stay apart — the three-rally-numbers rule applied to a bot's runs, which feed the same columns a human's do |
| `botSkill` | A bot's strength as a property of the ACCOUNT: derived from the id so it survives a restart, distinct across the roster, spread across the ladder rather than piled at the rails, mapped monotonically onto difficulty rungs, and — the load-bearing one — topping out UNDER the apex floor, so no bot reaches Overlord as a birthright. Plus that a fresh bot reads `0/5 Unranked`, which is the whole of "bots earn their tier" |
| `botRating` | What a cross-kind match is worth, walked over all FOUR pairings rather than the reduced case alone — a bug that scaled every PvP match would satisfy a one-sided check while quietly halving the whole ladder. Every duel is handed an identical opponent rating through the context, so `kindScale` is the only thing that differs, and the assertion is a RATIO rather than a literal mu. Plus that the match still COUNTS: XP, `rankedGames`, `rankedDuels` and the win all move, because reduced is not unranked |
| `bots` | The roster as data, after the reversal: that a seeded bot is handed nothing it has not played for, that it still LANDS on the board (bots keep their exemption from the rows-of-zeros filter — the fabricated career was the problem, not the row), and the `play_bots_v1` migration — an older build's pre-placed row is reset to unplaced, and a second boot does NOT take back the rating a bot has since earned (unflagged, every restart would be a silent demotion for the whole population) |
| `botPlayers` | The population against a real server, because `wss` lives inside `startServer` and a bot is observable only the way a player observes one. It boots a CONTROL server with the population OFF, so "tables appeared" cannot pass for some other reason, and checks the hosts are roster bots BY NAME. Without the in-process registry the relay would treat a bot as a cookieless socket — it would connect, then be refused a seat with `NEEDS_USERNAME`, and look like nothing at all from the outside |
| `queue` | Joining, pairing, seating and starting, against a real server |
| `duelRecord` `deviceSession` `accountRecovery` `accountDeletion` `roomLifecycle` `spectators` `queue` `tableBrowser` `cpuTable` `seatGate` `p2pParity` `headers` `botPlayers` | Thirteen suites that boot the real server (see §4) |
...
`streak` · `history` · `delete` · `report` · `eject` · `load` · `bots` · `bot-sim` · `build-id`
...
arrives between matches. A second leg does the same for the claim from inside: the control,
the promoted watcher landing in a playing seat rather than back on a dead court, the host coming
off the same overlay, and the handshake completing from a seat nobody joined.

**One assertion there is an OUTCOME check with two defences behind it, and the difference
matters when somebody edits one of them.** "The promoted watcher is not charged for the match
they only watched" is protected both by the lobby-return clearing `winner` before `spectating`
clears and by the record effect marking its ref instead of merely skipping; on that path either
alone suffices, so removing one leaves the suite green. Removing BOTH fires it, with
`played 0 → 1` and `xp 0 → 366`. It is worth keeping exactly as it is — the harm is what a
player would feel — but it pins the outcome, not either mechanism, and a comment claiming
otherwise was corrected rather than left standing. That last one is the only place in the suite where the old predicate and
...
below-the-next-floor property. Only Chaos's cap reaches Legend now: Cyber's dropped from 36.9
to 33.9 after a player farmed it to the cap and crossed the apex off a single even duel, since
0.1 under 37 is one performance-weighted win away from it. And the apex asks for
`OVERLORD_MIN_DUELS` ranked duels beyond the rating — `tests/rating.test.ts` pins `tierFor`
holding a μ40 record at Legend on 24 duels and promoting it on the 25th, and
`tests/apexGate.test.ts` pins which results COUNT: a stock duel in a venue that rates it, and
not a Casual table, party rules, a solo match or a replayed key. And solo XP carries momentum and fatigue, so a win streak is
...
**A finished task re-deals for free a fixed number of times a day, then its slot rests.** The
free re-deal used to be unlimited, and measured with the repo's own functions it paid 13.5k-29k
task XP a day at sixty matches — up to ~480 XP a match on top of the match — so tasks, not
matches, set the level pace and all six elite themes fell in one day. `tests/missions.test.ts`
pins `FREE_REDEALS_REGULAR` and `FREE_REDEALS_ELITE`, that the claim past the allowance leaves the
hand a slot short across a re-read (the sweep must not resurrect it), that both counters come back
whole the next UTC day, and that an elite task can be claimed at most twice a day — which is what
makes sixteen elite tasks at least eight calendar days. Prove-fail: drop the allowance check.
...
exactly where a vacuous pass hides. Reaching Overlord is still `rankMu >= 37` plus the player's OWN
ranked-duel count and nothing else — the headcount is a display, never a definition — so no test
here may make one player's tier depend on another's; the fixture seeds `rankedDuels` at the
