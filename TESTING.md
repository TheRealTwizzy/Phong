...
| `matchmaking` | Who the ranked queue pairs, and how hard it insists (pure) — including the three pair classes, the demand-sized fallback reservation, and the bot-vs-bot diversity preference on pass 3 alone. That last one needs the already-played partner to be the BEST-SCORING one or the un-predicated matcher picks the fresh one anyway and the case proves nothing — and the capped bot must sit INSIDE the band, which cost a first attempt: at a 2.0 mu gap `p` is 0.380, outside TIGHT, so the pairing was refused for a band reason rather than a diversity one. Two of its cases were vacuous as written: pair-class ties with every seat at the same mu resolved to queue order, so a type-blind matcher satisfied them, and no case discriminated pass 2 from pass 3, because the reservation empties pass 3 whenever an unpaired human is compatible with a bot |
...
| `playbotPolicy` `playbotPopulation` | What an autonomous bot chooses, and which existing bots should be playing (both pure, both floored at 100/95). The coverage floor found a duplicated rule rather than an untested line: the patience clause existed in two places, and one copy had no test. Plus the two rules that decide whether the population can climb AT ALL: that every tier — `unranked` through `overlord` — has a room in `OPEN_VENUES` it may enter AND that rates it, which nothing stated and which failed from Vanguard up; and that with NO band centre the ordering reads no rating, with the fixture built so the μ40 bot has played least, so restoring the old `START_MU` substitution reddens it. `chooseVenue`'s second draw needs a pool of three or more, which is exactly why no existing case could reach it: every one passed `['casual','beginner']` or `['casual']`, where any index is correct. Plus the roster-proportional idle share and the weighted action DRAW, and BOTH have a fixture trap of their own: `snap()`'s default roster of 20 sits below the crossover (`max(6, round(20 * 0.25))` is 6), so every pre-existing baseline case passes against the unscaled constant and says nothing — and a draw fixture with a clear argmax winner answers identically under both implementations, so the appetites must TIE |
...
| `db-wipe` `taskReset` `placementRescue` `rankedBackfill` `chaosRelabel` `shutoutRecount` `rankedDuelsBackfill` `advancedLadder` `botIdentity` `playbotRename` `rosterRetire` `playbotLadderReset` | The one-shot migrations. **A one-shot is invisible to a test that does not un-stamp it and re-import** — its key is already in `meta`, so nothing drives it and a mutation to its SQL reddens nothing. Clear the key, `vi.resetModules()`, re-import, then assert the repair — or, as `playbotRename` and `playbotLadderReset` do, hand-build the pre-migration database with every OTHER destructive key stamped and let the first import drive it. `playbotLadderReset` had to stamp `roster_retire_v1` as well, because that one DELETES the cookieless furniture row its own selector test depends on |
