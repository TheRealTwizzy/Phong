...
Progression is server-owned: daily missions live in the database on a UTC day key and are claimed by id, and achievements that mean "you beat something" pay out scaled by the same prediction the match XP uses. Achievements form a **branching tree** across five lines of play, and the tree is the progression gate: Pro unlocks by beating Rookie, Cyber by beating Pro, longer matches by building a career. Deep rungs stay hidden until you reach them. Every player locks in a **unique username** on first arrival (case-insensitive, held for 365 days per change) and can upload an **avatar** (auto-cropped to 256×256). Tap any username — leaderboard, match history, lobby, or mid-match — to view that player's public profile.
